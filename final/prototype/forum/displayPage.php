...
try {
    ...
    $forumData = getForumDetailInfo($file_db,$selectedId);
    $forumReplyData = getForumReplyDetailInfo($file_db,$selectedId);
  }
 ...
 if($_SERVER['REQUEST_METHOD'] == 'POST')
 {
   ...
   $insertStatement =  "INSERT INTO forumReply (forumId, userEmail, currentUser,content,createDate,createTime) VALUES ('" . $selectedId . "', '" . $userEmail . "', '" . $currentUser . "', '" . $content . "', '" . $createDate . "', '" . $createTime . "')";
   $file_db->exec($insertStatement);
   $url = "../forum/displayPage.php?selectedId=". $selectedId;
  header("Refresh:0");
}

   function getForumDetailInfo($file_db,$selectedId) {
   $sql_select='SELECT * FROM forum WHERE forumId =' .$selectedId;
   ...
   function getForumReplyDetailInfo($file_db,$selectedId) {
   $sql_select='SELECT * FROM forumReply WHERE forumId =' .$selectedId;
   $result = $file_db->query($sql_select);
   if (!$result) die("Cannot execute query.");
    $projects = [];
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
...
var forumData = <?php echo json_encode($forumData); ?>;
var forumReplyData = <?php echo json_encode($forumReplyData); ?>;
