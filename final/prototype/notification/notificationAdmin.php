...
if(isset($_SESSION['isLogin'])){
 ...
 try {
     ...
     $tableData = getNotificationInfo($file_db);
   }
   ...
   if($_SERVER['REQUEST_METHOD'] == 'POST')
   {
     ...
     $insertStatement =  "INSERT INTO notification (userEmail, currentUser,topic,content,createDate,createTime) VALUES ('" . $userEmail . "', '" . $currentUser . "', '" . $topic . "', '" . $content . "', '" . $createDate . "', '" . $createTime . "')";
     $file_db->exec($insertStatement);
    header("Refresh:0");
  }

 function getNotificationInfo($file_db) {
 $sql_select='SELECT * FROM notification';
...
<script src="../notification/notificationAdmin.js">
</script>
  ...
  <div class="container " id="replySection">
    <form name="form" action="" method="post" id="replyForm">
      <div class="form-group">
        <label for="Topic">Notification Topic</label>
      ...
      <button type="submit" class="btn btn-primary btn-md btn-block" >Post Notification</button>
    </form>
  </div>
